        ...
        <div class="w-[1220px] h-[670px] flex gap-5 p-5 rounded-3xl shadow-[0_1px_60px_-15px_rgba(0,0,0,0.3)]">
            <div class="w-1/2 h-full bg-gray-200 rounded-2xl overflow-hidden relative">
                <div class="h-full w-full bg-black/45 absolute z-10 flex flex-col justify-between">
                    <div class="p-8">
                        <img src="/public/asset/logo.png" class="w-10 logo-white" alt="">
                    </div>
                ...
                <img src="/public/asset/bg-login.jpg" alt="bg-login" class="w-[1237px] max-w-[1220px] absolute z-0 -top-24 -left-44" srcset="">
            </div>
            <div class="w-1/2 flex h-full py-5 justify-center">
                <form method="POST" action="/public/login" class="h-full w-[484px] flex flex-col justify-between">
                    <div><img src="/public/asset/logo.png" class="w-10" alt=""></div>
                    <div class="w-full">
                        ...
                        <div class="flex items-center bg-white rounded-lg border overflow-hidden">
                            ...
                            <img class="px-4 cursor-pointer opacity-50" src="/public/icon/linear/eye-slash.svg" onclick="toogleBtn()" alt="eye-slash" id="toogle-input">
                        </div>
                        <?php if (isset($error)) : ?>
                            <p class="text-red-500"><?= htmlspecialchars($error); ?></p>
                        <?php endif; ?>
                    </div>
                    <button type="submit" class="w-full bg-main-800 hover:bg-main-600 text-white pl-6 pr-6 hover:pr-4 py-3 rounded-lg font-semibold flex justify-between duration-200"><p>Login</p><img src="/public/icon/linear/arrow-right.svg" class="invert brightness-0" alt=""></button>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        function toogleBtn() {
            if (document.getElementById('toogle-input').alt == "eye-slash") {
                document.getElementById('toogle-input').src = '/public/icon/bold/eye.svg';
                document.getElementById('toogle-input').alt = "eye";
                document.getElementById('password').type = "text";
            } else if (document.getElementById('toogle-input').alt == "eye") {
                document.getElementById('toogle-input').src = '/public/icon/bold/eye-slash.svg';
